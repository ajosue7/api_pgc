...
@Entity
@Table(name = "tbl_sectores_campos_transversales",
        indexes = {@Index(name = "idx_codigo_sector_gob", columnList = "CODIGO_SECTOR")},
        uniqueConstraints = {@UniqueConstraint(columnNames = {"CODIGO_SECTOR"})})
public class TblSectorCampoTransversal {
    ...
    @ApiModelProperty(notes = "Codigo Campo Transversal", required = true)
    private String codigoSector;

    @Column(name = "NOMBRE_SECTOR", nullable = false, length = 250)
    @ApiModelProperty(notes = "Nombre del Campo Transversal")
    private String nombreSector;

    @Column(name = "DESCRIPCION_SECTOR", nullable = false, length = 250)
    @ApiModelProperty(notes = "Descripcion del Sector Campo Transversal")
    private String descripcionSector;

    @Column(name = "IMAGEN_SECTOR", nullable = false, length = 300)
    @ApiModelProperty(notes = "Imagen del Sector Campo Transversal")
    ...
    // Mapeo de la Relacion de la Tabla de Sectores Gobierno con Tipo Sector
    ...
    // Mapeo de la Relacion de la Tabla de Sectores Gobierno con Sectores OCDE Padre
    // Muchos Sector = 1 Sector Gobierno Padre
    @ManyToOne(cascade = CascadeType.ALL)
    @JoinColumn(name = "SECTOR_PADRE_ID", referencedColumnName = "ID_SECTOR")
    @ApiModelProperty(notes = "Entidad del Campo Transversal, se envia desde un Json (\"sectorPadreId\": { \"idSector\": \"valor\" })",
            required = true)
    private TblSectorCampoTransversal sectorPadreId;


    // Mapeo de la Relacion de la Tabla de Sectores Gobierno con Nivel de Sectores
    // Muchos Sector = 1 Nivel de Sector
    @ManyToOne(cascade = CascadeType.ALL)
    @JoinColumn(name = "ID_NIVEL_SECTOR", referencedColumnName = "ID_NIVEL_SECTOR")
    @ApiModelProperty(notes = "Entidad del Nivel de Campo Transversal, se envia desde un Json (\"idNivelSector\": { \"idNivelSector\": \"valor\" })",
            required = true)
     ...
     */
    public TblSectorCampoTransversal() {
    }
    ...
    public TblSectorCampoTransversal getSectorPadreId() {
        return sectorPadreId;
    }

    public void setSectorPadreId(TblSectorCampoTransversal sectorPadreId) {
